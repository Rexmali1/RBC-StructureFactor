...
def collision_detect(centroids, rotation_M, vel_cen_mass, omega, n): #Determines if a collision occurs and the points of contact 
  for u in range(n):
    for y in range(u):
...
centroids  = L*np.random.rand(n,3) #Centroid of ellipsoids
...
collision_detect(centroids, rotation_M, vel_cen_mass, omega, n)

for g in range(len(centroids)):
  for h in range(3):
    if centroids[g,h] < 0:
      centroids[g,h] = centroids[g,h] + L
    elif centroids[g,h] > L:
      centroids[g,h] = centroids[g,h] - L

p_s = []
p_s.append(centroids)

#Free movimient loop
for d in range(ite):

  centroids = centroids + h * vel_cen_mass
  
  angular_velocity(rotation_M, omega, h)
  ...
  if d%42 == 0:
    #Save the position of ellipsoid every 42 ms for the calculation of the structure factor.
    p_s.append(centroids)
